...
The last one is narrower than it looks, so state it precisely. The DLQ check is an AND:
`files.length() == 0 && title == ""`. A PR whose file fetch failed but whose title fetch
succeeded still reaches the worker, carrying a title and zero patches.

So the guarantee is: **the worker never receives a record that is completely empty.** It
can receive a partial one. `should_skip` in `reason.py` catches that case and writes the
row as `skipped` without calling the model. Two checks, in two places, because either one
